...
def train_mpscl(model, strain_loader,sval_loader,trgtrain_loader,cfg,epoch):
    """
        UDA training
    """
    ...
    for i_iter in tqdm(range(cfg.TRAIN.MAX_ITERS+1)):
        ...
        if i_iter % cfg.TRAIN.SAVE_PRED_EVERY == 0 and i_iter != 0:
            ...
            torch.save(model.state_dict(), snapshot_dir /  f'model_{epoch}_{i_iter}.pth')
            torch.save(d_aux.state_dict(), snapshot_dir /  f'model_{epoch}_{i_iter}_D_aux.pth')
            torch.save(d_main.state_dict(), snapshot_dir / f'model_{epoch}_{i_iter}_D_main.pth')
...
def train_domain_adaptation(model,strain_loader,trgtrain_loader,sval_loader,cfg,epoch):

    if cfg.TRAIN.DA_METHOD == 'MPSCL':
        train_mpscl(model, strain_loader, sval_loader,trgtrain_loader, cfg, epoch)
